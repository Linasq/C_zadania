...
void mean_variance(const int v[], int n, double *mean, double *variance) {
	if(n == 0) {
		...
		*variance = 0;
		...
		double sum = 0;
		for(int i = 0; i < n; sum+=v[i], i++); //calculating sum
	
		*mean = sum/n;

		double var=0;
		for(int i = 0; i < n; var += (v[i]-*mean) * (v[i]-*mean), i++);
...
void bernoulli_gen(int v[], int n, double probability) {
	for(int i = 0; i < n; v[i] = rand() < probability * (RAND_MAX + 1) ? 1 : 0, i++);
}
...
void pmf(double v[], int trials) {
	for(int i = 0; i < trials; i++) {
		int diceOne = rand_from_interval(1,6);
		int diceTwo = rand_from_interval(1,6);
		v[diceOne + diceTwo]++; // incrementing index of result by one
	}
	for(int i = 2; i <= 12; v[i]=v[i]/trials, i++); //calculating probability
}
...
void print_histogram(const double v[], int n, int x_start, double y_scale, char mark) {
	for(int i = x_start; i <= n+x_start; i++) {
		printf("%d |", i);
		int ctr = (int) round(v[i]/y_scale);

		for(int j = 0; j < ctr; j++) printf("%c", mark);
...
void monty_hall(int n, int *p_switch_wins) {
	int wins = 0;
	for(int i = 0; i < n; i++) {
		int doorNum = rand_from_interval(1,3);
		int playerNum = rand_from_interval(1,3);

		if(doorNum != playerNum) wins++;
		*p_switch_wins = wins;
	}
}
